            ...
            Form {
                ...
                if config.displayConsoleOnly {
                    ...
                    Section(header: Text("Graphics Card"), footer: Text("QEMU arguments for -device graphics card. For i386/x86_64, qxl-vga is set by default, for virt systems  ramfb is set by default.")) {
                        VMConfigStringPicker(selection: $config.graphicsCard, label: Text("Device"), rawValues: UTMConfiguration.supportedGraphicsCard(), displayValues: UTMConfiguration.supportedGraphicsCardPretty())
                    }
                }
            }
