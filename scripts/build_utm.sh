...
find "$BUILT_PATH" -type f -path '*/Frameworks/*.framework/*' -maxdepth 3 ! -name "Info.plist" | while read FILE; do lipo -info "$FILE" | grep "Non-fat file" > /dev/null || lipo -thin $ARCH "$FILE" -output "$FILE"; done
