...
### Comment Style
Comments are used to provide a high level summary of subsequent code or to document a design decision that is not obvious from the code. Avoid over-commenting which can pollute the code base. If you are writing more lines of comment than code, see if the comments can be simplified. Avoid: narrative comment that describes a development or debug process that led to the code being written. Avoid: adding a comment after removing code to "justify" the code that was removed. Avoid: any comment that refers to AI session internal jargon, identifiers, or progress tracking. Avoid: making similar comments in multiple places (for example making the same comment in both the header and source file).
...
- Backwards compatibility: Currently we support iOS 15+ and macOS 12+ although we tend to keep newer features locked to newer operating systems to reduce the work needed for testing. Any large SwiftUI change should be tested on a macOS 12 machine to make sure it works as expected on older, buggier, SwiftUI releases.
